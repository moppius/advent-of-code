...
def main(args=None):
    ...
    parser = ArgumentParser(description="Run Advent of Code 2020 challenges.")
    parser.add_argument("-y", "--year", help="choose the year")
    parser.add_argument("-d", "--day", help="run a specific day's challenge")
    args = parser.parse_args(args)
    run_challenges(args.year, args.day)


def run_challenges(year, only_day=None):
    """Run all challenges if `only_latest` is False, otherwise run the latest day's challenge."""

    days = [obj for obj in listdir(path.join(path.dirname(__file__), year)) if obj.startswith("day_")]
    found_day = False
    for day in days:
        if only_day is not None:
            if int(day.split("_")[1]) == int(only_day):
                found_day = True
                _run_challenge(year, day)
                break
        else:
            _run_challenge(year, day)
...
def _run_challenge(year, day):
    """Run the challenge for a given day."""

    name = f"Day {day.split('_')[1]}"
    print(f"\n\n{name}")
    module = import_module(f".{day}.{day}", year)
